...
export function screen(a: Answers): { pct: number; line: number; results: Result[] } {
  ...
  if (a.kids) {
    if (pct <= 200)
      out.push({ tone: 'free', who: 'Your kids', title: 'Your kids very likely qualify for free coverage.', body: `At your income, children in ${st.name} almost always qualify for Medicaid or CHIP. Doctor visits, checkups, shots, dental, and prescriptions are covered, usually at no cost. You can apply any day of the year.`, cta: [{ label: 'Apply for kids’ coverage (CHIP)', url: LINKS.chip }, applyAll], track: 'kids_free' });
    else if (pct <= 300)
  ...
  if (a.pregnant) {
    if (pct <= 200)
      out.push({ tone: 'free', who: 'Pregnancy', title: 'Your pregnancy is very likely covered for free.', body: `Every state covers pregnant people at this income through Medicaid, including prenatal care, delivery, and a full year after the birth in most states. Apply now, since coverage can start right away.`, cta: [applyAll, medicaidDirect], track: 'preg_free' });
    else
  ...
  if (a.senior) {
    out.push({ tone: pct <= 135 ? 'free' : 'low', who: 'Age 65 and older', title: pct <= 135 ? 'Medicare, plus help paying for it.' : 'Medicare is your coverage.', body: pct <= 135 ? 'At this income, a Medicare Savings Program or Medicaid can pay your Medicare premiums and most of your costs. Sign up for Medicare, then apply for the savings program.' : 'Most people get Part A free after working 10 years. Sign up around your 65th birthday to avoid late penalties.', cta: pct <= 135 ? [{ label: 'Get started with Medicare', url: LINKS.medicare }, { label: 'Medicare Savings Programs', url: LINKS.msp }] : [{ label: 'Get started with Medicare', url: LINKS.medicare }], track: 'senior' });
  }

  if (a.adults) {
    const limit = st.exp === 'full' ? 138 : st.exp === 'to100' ? 100 : 0;
    if (pct <= limit) {
      out.push({ tone: 'free', who: 'Adults', title: 'You very likely qualify for free Medicaid.', body: `${st.name} covers adults with income up to ${limit}% of the poverty line, and you are at about ${pct}%. Medicaid usually costs $0 per month, with little or nothing to pay at the doctor. You can apply any day of the year, and coverage can often pay recent medical bills from before you applied.${st.code === 'GA' ? ' Georgia’s program (Pathways) requires 80 hours a month of work, school, or volunteering.' : ''}`, cta: [applyAll, medicaidDirect], track: 'adult_medicaid' });
    } else if (pct < 100) {
      ...
      out.push({ tone: 'low', who: 'Adults', title: pct <= 150 ? 'You qualify for a big discount on a marketplace plan.' : 'You qualify for help paying for a marketplace plan.', body: `At about ${pct}% of the poverty line, a tax credit lowers your monthly premium, and some plans can cost $0 after the credit. ${pct <= 250 ? 'Silver plans also come with lower deductibles and copays at your income. ' : ''}Open Enrollment for 2027 runs November 1, 2026, to January 15, 2027. You can sign up sooner if you recently lost coverage, moved, married, or had a baby.`, cta: [{ label: `See prices at ${mkt.name}`, url: mkt.url }, { label: 'Can I sign up now?', url: LINKS.sep }], track: 'adult_subsidy' });
    } else {
      out.push({ tone: 'full', who: 'Adults', title: 'You can buy a marketplace plan at full price, or a cheaper plan today.', body: `Above 400% of the poverty line there is no premium tax credit. Compare marketplace plans during Open Enrollment (November 1 to January 15), or look at the lower-cost plans below if you need coverage now.`, cta: [{ label: `Compare plans at ${mkt.name}`, url: mkt.url }], track: 'adult_full' });
    }
  }
...
export const OFFERS: Offer[] = [
  { id: 'ehealth-stm', name: 'eHealth', what: 'Short-term health plans', line: 'Compare short-term plans from major insurers and often start coverage the next day.', url: pick(process.env.NEXT_PUBLIC_AFF_EHEALTH, 'https://www.ehealthinsurance.com/short-term-health-insurance'), env: 'NEXT_PUBLIC_AFF_EHEALTH', tag: 'Start as soon as tomorrow' },
  { id: 'pivot', name: 'Pivot Health', what: 'Short-term medical', line: 'Apply online in minutes for a short-term medical plan to bridge a gap in coverage.', url: pick(process.env.NEXT_PUBLIC_AFF_PIVOT, 'https://www.pivothealth.com/'), env: 'NEXT_PUBLIC_AFF_PIVOT', tag: 'Apply in minutes' },
  { id: 'dentalplans', name: 'DentalPlans.com', what: 'Dental savings plans', line: 'Save 10–60% at the dentist, and most plans can be used within 72 hours of joining.', url: pick(process.env.NEXT_PUBLIC_AFF_DENTALPLANS, 'https://www.dentalplans.com/'), env: 'NEXT_PUBLIC_AFF_DENTALPLANS', tag: 'Use within 72 hours' },
  { id: 'vsp', name: 'VSP Individual', what: 'Vision insurance', line: 'Individual vision plans for eye exams, glasses, and contacts, with no waiting period.', url: pick(process.env.NEXT_PUBLIC_AFF_VSP, 'https://www.vsp.com/'), env: 'NEXT_PUBLIC_AFF_VSP', tag: 'No waiting period' },
];
